...
using std::vector;
namespace tcam {
unsigned int PrefixSizeHelper(unsigned int low, unsigned int high,
                              unsigned int boundLow, unsigned int boundHigh) {
  if (boundHigh < low || boundLow > high) return 0;
  if (low <= boundLow && high >= boundHigh) return 1;

  unsigned int boundMid =
      (boundLow / 2) + (boundHigh / 2);  // Guard against overflow
  return PrefixSizeHelper(low, high, boundLow, boundMid) +
         PrefixSizeHelper(low, high, boundMid + 1, boundHigh);
}
