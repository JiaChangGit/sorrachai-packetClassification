...
class Utilities {
 ...
 public:
  ...
  static int GetMaxOverlap(const std::multiset<unsigned int>& lo,
                           const std::multiset<unsigned int>& hi);

  static std::pair<std::vector<int>, int> MWISIntervals(
      const std::vector<WeightedInterval>& I);
  static std::pair<std::vector<int>, int> FastMWISIntervals(
      const std::vector<LightWeightedInterval>& I);
  static std::pair<std::vector<int>, int> MWISIntervals(
      const std::vector<Rule>& I, int x);

  static std::vector<WeightedInterval> CreateUniqueInterval(
      const std::vector<Rule>& rules, int field);
  static std::vector<LightWeightedInterval> FastCreateUniqueInterval(
      const std::vector<interval>& rules);
  static std::vector<std::vector<WeightedInterval>>
  CreateUniqueIntervalsForEachField(const std::vector<Rule>& rules);
...
struct LightWeightedInterval {
  static const int LOW = 0;
  static const int HIGH = 1;
  LightWeightedInterval(unsigned int a, unsigned int b, int w)
      : a(a), b(b), w(w) {}
  unsigned int a;
  unsigned int b;
  int w;
  ...
  void Push(int x) { rule_indices.emplace_back(x); }
  std::vector<int> GetRuleIndices() const { return rule_indices; }
...
class WeightedInterval {
 ...
 public:
  WeightedInterval(const std::vector<Rule>& rules, unsigned int a,
                   unsigned int b)
      : rules(rules) {
    if (rules.size() == 0) {
      std::cout << "ERROR: EMPTY RULE AND CONSTRUCT INTERVAL?"
                << "\n";
      exit(0);
    }
  ...
  WeightedInterval(const std::vector<Rule>& rules, int field)
    ...
    ival =
        std::make_pair(rules[0].range[field][LOW], rules[0].range[field][HIGH]);
    SetWeightBySizePlusOne();
  }
  ...
  void SetWeightBySizePlusOne() { weight = rules.size() + 1; }
  void SetWeightBySize() { weight = rules.size(); }
