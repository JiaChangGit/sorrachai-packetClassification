...
std::pair<std::vector<SortableRulesetPartitioner::part>, bool>
SortableRulesetPartitioner::IsThisPartitionSortable(const part& apartition,
                                                    int current_field) {
  std::vector<WeightedInterval> wi =
      Utilities::CreateUniqueInterval(apartition, current_field);
  ...
  for (auto& w : wi) {
    ...
    for (auto& r : w.GetRules()) {
      temp_partition_rule.emplace_back(r);
    }
    new_partitions.emplace_back(temp_partition_rule);
  }
  return std::make_pair(new_partitions, Utilities::GetMaxOverlap(low, hi) == 1);
}
std::pair<std::vector<SortableRulesetPartitioner::part>, bool>
SortableRulesetPartitioner::IsEntirePartitionSortable(
...
bool SortableRulesetPartitioner::IsBucketReallySortable(
  ...
  int num_fs = 0;
  all_partitions.emplace_back(b.GetRule());
  int num_itr = 0;
...
std::pair<std::vector<SortableRulesetPartitioner::part>, int>
SortableRulesetPartitioner::MWISonPartition(const part& apartition,
                                            int current_field) {
  std::vector<WeightedInterval> wi =
      Utilities::CreateUniqueInterval(apartition, current_field);
  // printf("Field %d\n", current_field);
...
std::pair<std::vector<SortableRulesetPartitioner::part>, int>
SortableRulesetPartitioner::MWISonEntirePartition(
    const std::vector<part>& all_partition, int current_field) {
  std::vector<part> new_entire_partition;
  int sum_weight = 0;
  for (const auto& p : all_partition) {
...
void SortableRulesetPartitioner::BestFieldAndConfiguration(
  ...
  current_field.emplace_back(current_best_field);
}

std::pair<bool, std::vector<int>>
SortableRulesetPartitioner::GreedyFieldSelectionTwoIterations(
  ...
  all_partitions.emplace_back(rules);
  for (int i = 0; i < 2; i++) {
  ...
  for (int j = 0; j < num_fields; j++) {
    if (std::find(begin(current_field), end(current_field), j) ==
      ...
      current_field.emplace_back(j);
    }
  }
  return std::make_pair(current_rules.size() == rules.size(), current_field);
}
std::pair<std::vector<Rule>, std::vector<int>>
SortableRulesetPartitioner::GreedyFieldSelection(
    const std::vector<Rule>& rules) {
  if (rules.size() == 0) {
    printf("Warning: GreedyFieldSelection rule size = 0\n ");
  }
...
std::vector<SortableRuleset>
SortableRulesetPartitioner::SortableRulesetPartitioningGFS(
    const std::vector<Rule>& rules) {
  ...
  while (!current_rules.empty()) {
    ...
    if (rule_and_field_order.first.size() == 0) {
      printf(
          "Warning rule_and_field_order.first = 0 in "
          "SortableRulesetPartitioning\n");
      exit(0);
    }
    ...
    all_buckets.emplace_back(sb);
    current_rules.erase(remove_if(begin(current_rules), end(current_rules),
                                  [](const Rule& r) { return r.markedDelete; }),
                        end(current_rules));
  }
  return all_buckets;
}
...
std::vector<SortableRuleset>
SortableRulesetPartitioner::AdaptiveIncrementalInsertion(
    const std::vector<Rule>& rules, int threshold) {
  std::vector<SortableRuleset> all_buckets;

  auto InsertRuleIntoAllBucket = [threshold](const Rule& r,
                                             std::vector<SortableRuleset>& b) {
    for (auto& bucket : b) {
      if (bucket.InsertRule(r)) {
        if (bucket.size() < threshold) {
          ...
          auto result =
              SortableRulesetPartitioner::GreedyFieldSelection(temp_rules);
...
int SortableRulesetPartitioner::ComputeMaxIntersection(
  ...
  if (ruleset.size() == 0)
    printf(
        "Warning SortableRulesetPartitioner::ComputeMaxIntersection: Empty "
        "ruleset\n");
...
int SortableRulesetPartitioner::ComputeMaxIntersectionRecursive(
  ...
  for (int i = 0; i < (int)rules.size(); i++) {
    start_pointx.emplace_back(
        std::make_pair(rules[i].range[field_order[field_depth]][LOW], i));
    end_pointx.emplace_back(
        std::make_pair(rules[i].range[field_order[field_depth]][HIGH], i));
    queries.emplace_back(rules[i].range[field_order[field_depth]][LOW]);
    queries.emplace_back(rules[i].range[field_order[field_depth]][HIGH]);
  }
  ...
  for (unsigned int qx : queries) {
    ...
    if (start_pointx.size() != i_s) {
      while (qx == start_pointx[i_s].first) {
        ...
        rule_this_loop.emplace_back(rules[start_pointx[i_s].second]);
        i_s++;
        justInserted = 1;
    ...
    if (justInserted) {
      int this_POM = ComputeMaxIntersectionRecursive(
          rule_this_loop, field_depth - 1, field_order);
...
std::vector<SortableRuleset>
SortableRulesetPartitioner::MaximumIndepdenentSetPartitioning(
    const std::vector<Rule>& rules) {
  std::vector<Rule> current_rules = rules;
  std::vector<SortableRuleset> all_buckets;
  int sum_rank = 0;
  while (!current_rules.empty()) {
    ...
    all_buckets.emplace_back(SortableRuleset(temp_rules, field_order));
    current_rules.erase(remove_if(begin(current_rules), end(current_rules),
                                  [](const Rule& r) { return r.markedDelete; }),
                        end(current_rules));
  }
...
std::pair<std::vector<Rule>, std::vector<int>>
SortableRulesetPartitioner::MaximumIndependentSetAllFields(
    const std::vector<Rule>& rules) {
...
std::vector<Rule> SortableRulesetPartitioner::MaximumIndependentSetGivenField(
  ...
  return MaximumIndependentSetGivenFieldRecursion(rules, fields, 0, 0, 0)
      .GetRules();
}

WeightedInterval
SortableRulesetPartitioner::MaximumIndependentSetGivenFieldRecursion(
    const std::vector<Rule>& rules, const std::vector<int>& fields, int depth,
    unsigned a, unsigned b) {
  if (a > b) {
    printf("Error: interval [a,b] where a > b???\n");
    exit(1);
  }
  std::vector<WeightedInterval> vwi =
      Utilities::CreateUniqueInterval(rules, fields[depth]);
  std::vector<WeightedInterval> vwi_opt;
  if (depth < rules[0].dim - 1) {
    for (auto& wi : vwi) {
      vwi_opt.emplace_back(MaximumIndependentSetGivenFieldRecursion(
          wi.GetRules(), fields, depth + 1, wi.GetLow(), wi.GetHigh()));
    }
  } else {
    vwi_opt = vwi;
  }
...
//////////
std::pair<std::vector<SortableRulesetPartitioner::part>, int>
SortableRulesetPartitioner::FastMWISonPartition(const part& apartition,
                                                int field) {
  ...
  std::vector<LightWeightedInterval> wi =
      Utilities::FastCreateUniqueInterval(rules_given_field);
  ...
  for (int i : mwis.first) {
    ...
    for (auto& ind : wi[i].GetRuleIndices()) {
      temp_partition_rule.emplace_back(apartition[rules_given_field[ind].id]);
    }
    new_partitions.emplace_back(temp_partition_rule);
  }
...
std::pair<std::vector<SortableRulesetPartitioner::part>, int>
SortableRulesetPartitioner::FastMWISonEntirePartition(
    const std::vector<part>& all_partition, int current_field) {
  std::vector<part> new_entire_partition;
  int sum_weight = 0;
  for (const auto& p : all_partition) {
...
std::pair<bool, std::vector<int>>
SortableRulesetPartitioner::FastGreedyFieldSelectionTwoIterations(
    const std::vector<Rule>& rules) {
  if (rules.size() == 0) {
    printf("Warning: GreedyFieldSelection rule size = 0\n ");
  }
...
std::pair<std::vector<Rule>, std::vector<int>>
SortableRulesetPartitioner::FastGreedyFieldSelection(
    const std::vector<Rule>& rules) {
  if (rules.size() == 0) {
    printf("Warning: GreedyFieldSelection rule size = 0\n ");
  }
