...
typedef struct rb_red_blk_tree {
  ...
  void PopPriority(int p) {
    ...
    if (p == max_priority_local) {
      max_priority_local =
          *std::max_element(begin(priority_list), end(priority_list));
    }
  ...
  int GetSizeList() const { return priority_list.size(); }
} rb_red_blk_tree;
...
std::vector<std::pair<rb_red_blk_tree*, rb_red_blk_node*>> RBFindNodeSequence(
    rb_red_blk_tree* tree, const std::vector<box>& key, int level,
    const std::vector<int>& fieldOrder);
...
bool TreeInsertWithPathCompressionHelp(rb_red_blk_tree* tree,
                                       ...
                                       const std::vector<int>& fieldOrder,
                                       int priority, rb_red_blk_node*& out_ptr);
...
void RBSerializeIntoRulesRecursion(rb_red_blk_tree* tree, rb_red_blk_node* node,
                                   int level,
                                   const std::vector<int>& fieldOrder,
