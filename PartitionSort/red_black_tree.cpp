...
int inline CompareQuery(const box& a, const Packet& q, int level,
...
rb_red_blk_tree* RBTreeCreate() {
  ...
  newTree = new rb_red_blk_tree;  //(rb_red_blk_tree*)
                                  // SafeMalloc(sizeof(rb_red_blk_tree));
...
bool RBTreeCanInsert(rb_red_blk_tree* tree, const std::vector<box>& z,
                     int level, const std::vector<int>& fieldOrder) {
  ...
  if (level == fieldOrder.size()) {
    ...
    for (size_t i = level; i < fieldOrder.size(); i++) {
      if (IsIdentical(z[fieldOrder[i]][0], z[fieldOrder[i]][1],
                      chain_boxes[i - level][0], chain_boxes[i - level][1]))
        continue;
      if (IsIntersect(z[fieldOrder[i]][0], z[fieldOrder[i]][1],
                      chain_boxes[i - level][0], chain_boxes[i - level][1]))
        return false;
      else
        return true;
  ...
  while (x != nil) {
    ...
    if (compare_result == 1) { /* x.key > z.key */
      ...
      return true;
      x = x->right;*/
      return level == z.size() - 1 ? true
                                   : RBTreeCanInsert(x->rb_tree_next_level, z,
                                                     level + 1, fieldOrder);
    } else { /* x.key || z.key */
      return false;
    }
...
bool TreeInsertWithPathCompressionHelp(rb_red_blk_tree* tree,
                                       rb_red_blk_node* z,
                                       const std::vector<box>& b, int level,
                                       const std::vector<int>& fieldOrder,
                                       int priority,
    ...
    if (compare_result == 1) { /* x.key > z.key */
      ...
      if (level != fieldOrder.size() - 1) {
        /*if (x->rb_tree_next_level->count == 1) {
                ...
                while ( (temp_chain_boxes[run][0] == b[fieldOrder[level +
        run]][0] && temp_chain_boxes[run][1] == b[fieldOrder[level + run]][1]))
        { x->rb_tree_next_level = RBTreeCreate(); x->rb_tree_next_level->count =
        1; x = RBTreeInsert(x->rb_tree_next_level, temp_chain_boxes, level +
        run, naturalFieldOrder, priority); run++; if (level + run >=
        fieldOrder.size()) break;
                }
                ...
                else if (!(temp_chain_boxes[run][0] == b[fieldOrder[level +
        run]][0] && temp_chain_boxes[run][1] == b[fieldOrder[level + run]][1]))
        {
                        //split into z and x node
                        x->rb_tree_next_level = RBTreeCreate();
                        RBTreeInsert(x->rb_tree_next_level, temp_chain_boxes,
        level + run, naturalFieldOrder, xpriority);
                        RBTreeInsert(x->rb_tree_next_level, b, level + run,
        fieldOrder, priority);
                }

        }
        else {*/
        RBTreeInsertWithPathCompression(x->rb_tree_next_level, b, level + 1,
                                        fieldOrder, priority);
        //}
...
rb_red_blk_node* RBTreeInsertWithPathCompression(
    rb_red_blk_tree* tree, const std::vector<box>& key, unsigned int level,
  ...
  if (tree->count == 0) {
    ...
    // level <= b.size() -1
    for (size_t i = level; i < fieldOrder.size(); i++)
      tree->chain_boxes.emplace_back(key[fieldOrder[i]]);
    return nullptr;
  }
  if (tree->count == 1) {
    // path compression
    ...
    std::iota(begin(naturalFieldOrder), end(naturalFieldOrder), 0);
    size_t run = 0;
    if (temp_chain_boxes[run][0] == key[fieldOrder[level + run]][0] &&
        temp_chain_boxes[run][1] == key[fieldOrder[level + run]][1]) {
      //  printf("[%u %u] vs. [%u %u]\n", temp_chain_boxes[run][0],
      //  temp_chain_boxes[run][1], key[fieldOrder[level + run]][0],
      //  key[fieldOrder[level + run]][1]);

      x = RBTreeInsert(tree, key, level + run++, fieldOrder, xpriority);
      if (level + run < fieldOrder.size()) {
        while ((temp_chain_boxes[run][0] == key[fieldOrder[level + run]][0] &&
                temp_chain_boxes[run][1] == key[fieldOrder[level + run]][1])) {
          x->rb_tree_next_level = RBTreeCreate();
          ...
          x = RBTreeInsert(x->rb_tree_next_level, key, level + run, fieldOrder,
                           priority);
      ...
      if (level + run >= fieldOrder.size()) {
        ...
        x->rb_tree_next_level->count++;
        x->rb_tree_next_level->PushPriority(xpriority);
      } else if (!(temp_chain_boxes[run][0] ==
                       key[fieldOrder[level + run]][0] &&
                   temp_chain_boxes[run][1] ==
                       key[fieldOrder[level + run]][1])) {
        if (IsIntersect(temp_chain_boxes[run][0], temp_chain_boxes[run][1],
                        key[fieldOrder[level + run]][0],
                        key[fieldOrder[level + run]][1])) {
          printf("Warning not intersect?\n");
          printf("[%lu %lu] vs. [%lu %lu]\n", temp_chain_boxes[run][LowDim],
                 temp_chain_boxes[run][HighDim],
                 key[fieldOrder[level + run]][LowDim],
                 key[fieldOrder[level + run]][HighDim]);
          printf("chain_boxes:\n");
          for (auto e : temp_chain_boxes)
            printf("[%u %u] ", e[LowDim], e[HighDim]);
          printf("\n boxes:\n");
          for (size_t i = 0; i < key.size(); i++) {
            printf("[%u %u] ", key[fieldOrder[i]][LowDim],
                   key[fieldOrder[i]][HighDim]);
          }
          printf("\n");
          exit(0);
        }
        ...
        RBTreeInsertWithPathCompression(
            z1->rb_tree_next_level, PrependChainbox(temp_chain_boxes, level),
      ...
      auto z1 = RBTreeInsert(tree, PrependChainbox(temp_chain_boxes, level),
                             level + run, naturalFieldOrder, xpriority);
...
rb_red_blk_node* RBTreeInsert(rb_red_blk_tree* tree,
                              const std::vector<box>& key, int level,
...
bool TreeInsertHelp(rb_red_blk_tree* tree, rb_red_blk_node* z,
  ...
  while (x != nil) {
    ...
    if (compare_result == 1) { /* x.key > z.key */
      ...
      printf("x:[%u %u], z:[%u %u]\n", x->key[LowDim], x->key[HighDim],
             z->key[LowDim], z->key[HighDim]);
...
void InorderTreePrint(rb_red_blk_tree* tree, rb_red_blk_node* x) {
  ...
  if (x != tree->nil) {
    ...
    // tree->PrintKey(x->parent->key); printf("  red=%i\n",x->red);
...
void TreeDestHelper(rb_red_blk_tree* tree, rb_red_blk_node* x) {
  rb_red_blk_node* nil = tree->nil;
  if (x != nil) {
    if (x->rb_tree_next_level != nullptr) RBTreeDestroy(x->rb_tree_next_level);
...
int RBExactQuery(rb_red_blk_tree* tree, const Packet& q, int level,
                 const std::vector<int>& fieldOrder) {
...
int RBExactQueryPriority(rb_red_blk_tree* tree, const Packet& q, int level,
                         const std::vector<int>& fieldOrder,
                         int priority_so_far) {
  // printf("entering level %d - tree->GetMaxPriority =%d\n",
  // level,tree->GetMaxPriority());
...
void RBTreeDeleteWithPathCompression(rb_red_blk_tree*& tree,
  ...
  if (tree->count == 2) {
    ...
    //	std::stack<std::pair<rb_red_blk_tree *, rb_red_blk_node *>>
    // stack_so_far;
    ...
    while (true) {
      ...
      if (x->left == temp_tree->nil && x->right == temp_tree->nil) {
        tree->chain_boxes.emplace_back(x->key);
        ...
        // stack_so_far.push(std::make_pair(temp_tree, x));
        if (compare_result == 0) {  // hit top = delete top then go leaf node to
                                    // collect correct chain box
          if (x->left == temp_tree->nil) {
            temp_tree = x->right->rb_tree_next_level;
            tree->chain_boxes.emplace_back(x->right->key);
          } else {
            temp_tree = x->left->rb_tree_next_level;
            tree->chain_boxes.emplace_back(x->left->key);
          }
          //	tree->chain_boxes.emplace_back(x->left ==
          // temp_tree->nil?x->right->key:x->left->key); 	temp_tree =
          // x->rb_tree_next_level;
        } else {
          temp_tree = x->rb_tree_next_level;
          tree->chain_boxes.emplace_back(x->key);
        }
        ...
        /*if (compare_result == -1){  root < z.key
                        //delete right child
                        //stack_so_far.push(std::make_pair(temp_tree,
        x->right)); tree->chain_boxes.emplace_back( x->key); temp_tree =
        x->right->rb_tree_next_level; } else if(compare_result == 1) {
                        //delete left child
                        //stack_so_far.push(std::make_pair(temp_tree, x->left));
                        temp_tree = x->left->rb_tree_next_level;
                        tree->chain_boxes.emplace_back(x->key);
        } else {
                printf("Warning::RBTreeDeleteWithPathCompression Overlap at
        level %d\n",level+run);
        }*/
      }
      run++;
  ...
  while (x != nil) {
    ...
    if (compare_result == 1) { /* x.key > z.key */
      ...
      tree->count--;
      RBTreeDeleteWithPathCompression(x->rb_tree_next_level, key, level + 1,
      ...
      printf("x:[%u %u], key:[%u %u]\n", x->key[LowDim], x->key[HighDim],
             key[fieldOrder[level]][LowDim], key[fieldOrder[level]][HighDim]);
      printf("Warning RBFindNodeSequence : x.key || key[fieldOrder[level]]\n");
    }
  }

  printf(
      "Error RBTreeDeleteWithPathCompression: can't find a node at level %d\n",
      level);
  exit(0);
}
/***********************************************************************/
...
void RBSerializeIntoRulesRecursion(rb_red_blk_tree* treenode,
                                   rb_red_blk_node* node, int level,
                                   const std::vector<int>& fieldOrder,
                                   std::vector<box>& box_so_far,
  ...
  box_so_far.emplace_back(node->key);
...
int CalculateMemoryConsumptionRecursion(rb_red_blk_tree* treenode,
  ...
  return memory_usage + intervalsize +
         (number_pointers_in_internal_node + also_max_priority_interval_node) *
             sizeofint +
