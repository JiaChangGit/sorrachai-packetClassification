...
enum RequestType { ClassifyPacket, Insertion, Deletion };
struct Request {
...
struct Bookkeeper {
  ...
  void InsertRule(const Rule& r) { rules.emplace_back(r); }
...
class Simulator {
 public:
  ...
  Simulator(const std::vector<Rule>& ruleset,
            const std::vector<Packet>& packets)
      : ruleset(ruleset), packets(packets) {}

  static int PerformPartitioning(PartitionPacketClassifier& ppc,
  ...
  std::vector<int> PerformPacketClassification(
      PacketClassifier& classifier, const std::vector<Request>& sequence,
      std::map<std::string, double>& trial) const;

 private:
  std::vector<Request> GenerateRequests(int num_packet, int num_insert,
