...
struct Rule {
  // Rule(){};
  Rule(int dim = 5) : dim(dim), range(dim, {{0, 0}}), prefix_length(dim, 0) {
    markedDelete = 0;
  }
...
struct EndPoint {
  ...
  bool operator<(const EndPoint& rhs) const { return val < rhs.val; }
  double val;
  bool isRightEnd;
  int id;
};
...
enum TestMode { ModeClassification, ModeUpdate, ModeValidation };
