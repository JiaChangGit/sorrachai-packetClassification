...
/*
./main f="./ruleset/acl1_100k" p="./trace/acl1_100k_trace" c="All"
m="Classification" o="info/acl1_100k_log.csv"

./main f="./ruleset/acl1_100k" p="./trace/acl1_100k_trace" c="All" m="Update"
o="info/acl1_100k_up.csv"

./main f="./ruleset/acl1_100k" p="./trace/acl1_100k_trace" c="All"
m="Validation" o="info/acl1_100k_val.csv"
 */
using namespace std;
...
RunSimulatorOnlyClassification(const unordered_map<string, string>& args,
                               const vector<Packet>& packets,
                               const vector<Rule>& rules, ClassifierTests tests,
...
pair<vector<string>, vector<map<string, string>>> RunSimulatorUpdates(
    const unordered_map<string, string>& args, const vector<Packet>& packets,
    const vector<Rule>& rules, ClassifierTests tests, const string& outfile,
    int repetitions = 1);
...
int main(int argc, char* argv[]) {
  ...
  string filterFile = GetOrElse(args, "f", "./ruleset/acl1_100k");
  ...
  // set by default
  ClassifierTests classifier = ParseClassifier(GetOrElse(args, "c", "All"));
  ...
  switch (mode) {
    case ModeClassification:
      RunSimulatorOnlyClassification(args, packets, rules, classifier,
                                     outputFile);
      break;
    case ModeUpdate:
...
RunSimulatorOnlyClassification(const unordered_map<string, string>& args,
  ...
  vector<string> header = {
      "Classifier",  "ConstructionTime(ms)", "ClassificationTime(s)",
      "Size(bytes)", "MemoryAccess",         "Tables",
