...
static inline struct cmap_node *cmap_node_next(const struct cmap_node *node) {
  return node->next;
}

static inline struct cmap_node *cmap_node_next_protected(
...
#define CMAP_NODE_FOR_EACH_PROTECTED(NODE, MEMBER, CMAP_NODE)             \
  for (INIT_CONTAINER(NODE, CMAP_NODE, MEMBER);                           \
       (NODE) != OBJECT_CONTAINING(NULL, NODE, MEMBER); ASSIGN_CONTAINER( \
           NODE, cmap_node_next_protected(&(NODE)->MEMBER), MEMBER))
...
#define CMAP_CURSOR_FOR_EACH__(NODE, CURSOR, MEMBER)               \
  ((CURSOR)->node ? (INIT_CONTAINER(NODE, (CURSOR)->node, MEMBER), \
                     cmap_cursor_advance(CURSOR), true)            \
                  : false)
 ...
 * 'cmap' is empty. */
static inline struct cmap_node *cmap_first(const struct cmap *cmap) {
...
static inline size_t cmap_remove(struct cmap *cmap, struct cmap_node *node,
                                 uint32_t hash) {
  return cmap_replace(cmap, node, nullptr, hash);
}

/*
 * Calculates the size of the longest chain of nodes in the map which might have
 * to be searched Added by James Daly
 */
