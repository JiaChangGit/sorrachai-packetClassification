...
#define OVS_U128_MAX                  \
  (ovs_u128) {                        \
    .u64 = { UINT64_MAX, UINT64_MAX } \
  }
#define OVS_BE128_MAX                      \
  (ovs_be128) {                            \
    .be64 = { OVS_BE64_MAX, OVS_BE64_MAX } \
  }
...
static inline uint32_t hash_words_inline(const uint32_t p[], size_t n_words,
                                         uint32_t basis) {
  uint32_t hash;
  size_t i;
...
static inline uint32_t hash_words64_inline(const uint64_t p[], size_t n_words,
                                           uint32_t basis) {
  uint32_t hash;
  size_t i;
 ...
 * This is inlined for the compiler to have access to the 'n_words', which
 * in many cases is a constant. */
static inline uint32_t hash_words_inline(const uint32_t p_[], size_t n_words,
                                         uint32_t basis) {
...
#if __GNUC__ >= 4
static inline uint32_t hash_words(const uint32_t p[], size_t n_words,
                                  uint32_t basis) {
...
static inline uint32_t hash_words64(const uint64_t p[], size_t n_words,
                                    uint32_t basis) {
  if (__builtin_constant_p(n_words)) {
