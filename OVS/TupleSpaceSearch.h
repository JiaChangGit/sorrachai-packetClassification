...
struct Tuple {
 public:
  Tuple(const std::vector<int>& dims, const std::vector<unsigned int>& lengths,
        const Rule& r)
      : dims(dims), lengths(lengths) {
    for (int w : lengths) {
      tuple.emplace_back(w);
    }
    cmap_init(&map_in_tuple);
    Insertion(r);
  }
  //~Tuple() { Destroy(); }
  void Destroy() { cmap_destroy(&map_in_tuple); }
  ...
  Memory MemSizeBytes(Memory ruleSizeBytes) const {
    ...
    // return table.size() * ruleSizeBytes + table.bucket_count() *
    // POINTER_SIZE_BYTES;
  }
...
class TupleSpaceSearch : public PacketClassifier {
 public:
  ...
  int MemoryAccess() const { return WorstAccesses(); }
  ...
  void PlotTupleDistribution() {
    ...
    auto cmp = [](const std::pair<unsigned int, Tuple>& lhs,
                  const std::pair<unsigned int, Tuple>& rhs) {
  ...
  size_t NumTables() const { return all_tuples.size(); }
