...
vector<vector<unsigned int>> InputReader::ReadPackets(const string& filename) {
  ...
  while (getline(input_file, content)) {
    ...
    for (int i = 0; i < dim; i++) {
      one_packet.emplace_back(atoui(tokens[i]));
    }
    packets.emplace_back(one_packet);
    line_number++;
  }
  return packets;
}

void InputReader::ReadIPRange(std::array<unsigned int, 2>& IPrange,
  ...
  for (int i = 0; i < 4; i++) ptrange[i] = atoui(split_ip[i]);
  ...
  /*count the start IP */
  for (int i = 3; i > 3 - masklit1; i--) ptrange[i] = 0;
  if (masklit2 != 0) {
    masklit3 = 1;
    masklit3 <<= masklit2;
    masklit3 -= 1;
  ...
  // key += std::bitset<32>(IPrange[0] >> prefix_length).to_string().substr(32 -
  // prefix_length);
  /*count the end IP*/
  for (int i = 3; i > 3 - masklit1; i--) ptrange[i] = 255;
  if (masklit2 != 0) {
    masklit3 = 1;
    masklit3 <<= masklit2;
    masklit3 -= 1;
...
void InputReader::ReadPort(std::array<unsigned int, 2>& Portrange,
                           const string& from, const string& to) {
  Portrange[LOW] = atoui(from);
  Portrange[HIGH] = atoui(to);
}

void InputReader::ReadProtocol(std::array<unsigned int, 2>& Protocol,
...
int InputReader::ReadFilter(vector<string>& tokens, vector<Rule>& ruleset,
  ...
  /*allocate a few more bytes just to be on the safe side to avoid overflow
   * etc*/
  Rule temp_rule(dim);
  string key;
  if (tokens[0].at(0) != '@') {
  ...
  ruleset.emplace_back(temp_rule);
...
vector<Rule> InputReader::ReadFilterFileClassBench(const string& filename) {
  ...
  cout << "field " << i << " has " << iv.size() << " unique intervals" << "\n";
  }*/
  /*for (auto& r : rules) {
  for (auto &p : r.range) {
  cout << p[0] << ":" << p[1] << " ";
...
bool IsPower2(unsigned int x) { return ((x - 1) & x) == 0; }
...
void InputReader::ParseRange(std::array<unsigned int, 2>& range,
                             const string& text) {
...
vector<Rule> InputReader::ReadFilterFileMSU(const string& filename) {
  ...
  for (size_t i = 0; i < parts.size(); i++) {
    ...
    // PrefixLength(bounds[i][LOW], bounds[i][HIGH]));
  }
  ...
  while (getline(input_file, content)) {
    ...
    for (size_t i = 0; i < split_comma.size() - 1; i++) {
      ...
      if (IsPrefix(temp_rule.range[i][LOW], temp_rule.range[i][HIGH])) {
        temp_rule.prefix_length[i] =
            PrefixLength(temp_rule.range[i][LOW], temp_rule.range[i][HIGH]);
      }
      // if ((i == FieldSA || i == FieldDA) & !IsPrefix(temp_rule.range[i][LOW],
      // temp_rule.range[i][HIGH])) { 	printf("Field is not a prefix!\n");
      // }
...
vector<Rule> InputReader::ReadFilterFile(const string& filename) {
  ...
  if (content[0] == '!') {
    ...
    cout << "ERROR: unknown input format please use either MSU format or "
            "ClassBench format"
         << "\n";
    exit(1);
  }
