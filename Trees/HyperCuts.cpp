...
void PrintRange(const Range& r) { printf("[%u - %u]", r.low, r.high); }
  ...
  for (size_t d = 0; d < node->bounds.size(); d++) {
    ...
    for (const Rule* r : node->classifier) {
      if (r->range[d][LowDim] < node->bounds[d].low) {
        f.emplace_back(node->bounds[d].low);
      } else {
        f.emplace_back(r->range[d][LowDim]);
      }
      if (r->range[d][HighDim] > node->bounds[d].high) {
        f.emplace_back(node->bounds[d].high);
      } else {
        f.emplace_back(r->range[d][HighDim]);
      }
    }
    f.sort();
...
void SetBoundary(HyperCutsNode* src, HyperCutsNode* dest,
  ...
  for (size_t d = 0; d < src->bounds.size(); d++) {
    ...
    dest->cuts.emplace_back(-1);
  }
}

bool IsPresent(const vector<Range>& bounds, const Rule* rule) {
  for (size_t d = 0; d < bounds.size(); d++) {
    if (rule->range[d][HighDim] < bounds[d].low ||
        rule->range[d][LowDim] > bounds[d].high) {
      return false;
    }
  }
  return true;
  ...
  for (auto i1 = newlist.begin(); i1 != newlist.end(); i1++) {
    ...
    while (i2 != newlist.end()) {
      if (AreSameRules((*i1)->classifier, (*i2)->classifier)) {
        for (size_t d = 0; d < node->bounds.size(); d++) {
          (*i1)->bounds[d].low =
              min((*i1)->bounds[d].low, (*i2)->bounds[d].low);
          (*i1)->bounds[d].high =
              max((*i1)->bounds[d].high, (*i2)->bounds[d].high);
        }
        listing[*i2] = *i1;
        delete (*i2);
        i2 = newlist.erase(i2);
...
void MoveRulesUp(HyperCutsNode* node) {
  list<Rule*> rulesMovedUp =
      node->children.front()->classifier;  // Start with this set
  ...
  if (emptyIntersect) {
    for (Rule* r : rulesMovedUp) {
      for (HyperCutsNode* child : node->children) {
        child->classifier.erase(
            find(child->classifier.begin(), child->classifier.end(), r));
      }
      // Update statistics
    }
  }
...
void HyperCutsHelper::CalcDimensionsToCut(HyperCutsNode* node,
  ...
  for (size_t d = 0; d < node->bounds.size(); d++) {
    ...
    for (Rule* rule : node->classifier) {
      ...
      if (!found) {
        rangeList.emplace_back(check);
      }
    }
    uniqueElements.emplace_back(rangeList.size());
  }
...
list<HyperCutsNode*> HyperCutsHelper::CalcNumCuts1D(HyperCutsNode* root,
                                                    size_t dim) {
  int nump = 0;
    ...
    for (int k = 0; k < 2; k++) {
      ...
      for (Rule* r : node->classifier) {
        if (IsPresent(child->bounds, r)) {
          child->classifier.emplace_back(r);
        }
      }
...
list<HyperCutsNode*> HyperCutsHelper::CalcNumCuts2D(HyperCutsNode* root,
                                                    size_t* dims) {
  // row / col
  Point spans[2];
  int nump[2];
...
HyperCutsNode* HyperCutsHelper::CreateTree(const list<Rule*>& classifier) {
  ...
  for (size_t i = 0; i < root->bounds.size(); i++) {
    root->cuts.emplace_back(1);
  }

  root->classifier.insert(root->classifier.end(), classifier.begin(),
                          classifier.end());
  ...
  while (!worklist.empty()) {
    ...
    for (HyperCutsNode* n : childList) {
      ...
      /*for (size_t i = 0; i < n->bounds.size(); i++) {
              n->cuts.emplace_back(0);
      }*/
      node->children.emplace_back(n);
      node->childArray.emplace_back(n);
    }
    childList.clear();
    ...
    for (HyperCutsNode* n : toPush) {
      if (n->classifier.size() > leafSize) {
        ...
        for (size_t d = 0; d < n->bounds.size(); d++) {
          if (n->bounds[d].low != node->bounds[d].low ||
              n->bounds[d].high != node->bounds[d].high) {
            areIdentical = false;
          }
        }
  ...
  if (!node->childArray.empty()) {
    ...
    for (size_t d = 0; d < node->bounds.size(); d++) {
      if (node->cuts[d] > 1) {
        ...
        // node->bounds[d].high); printf("node->cuts = %d\n", node->cuts[d]);
    ...
    //	bool isNode = true;
    //	for (size_t d = 0; d < n->bounds.size(); d++) {
    //		if (n->bounds[d].low > packet[d] || n->bounds[d].high <
    // packet[d]) { 			isNode = false;
    // break;
    //		}
    //	}
    //	if (isNode) {
