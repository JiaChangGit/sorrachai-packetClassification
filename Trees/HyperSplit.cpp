...
vector<Rule> LeftList(const vector<Rule>& rules, int dim,
                      unsigned int splitPoint) {
  vector<Rule> leftList;
  for (const Rule& r : rules) {
    if (r.range[dim][LowDim] <= splitPoint) {
      leftList.emplace_back(r);
    }
  }
  return leftList;
}

vector<Rule> RightList(const vector<Rule>& rules, int dim,
                       unsigned int splitPoint) {
  vector<Rule> rightList;
  for (const Rule& r : rules) {
    if (r.range[dim][HighDim] > splitPoint) {
      rightList.emplace_back(r);
    }
  }
  return rightList;
}

vector<unsigned int> GetSplitPoints(const vector<vector<unsigned int>>& bounds,
                                    const vector<Rule>& rules, int dim) {
...
vector<Range> GetRanges(const vector<vector<unsigned int>>& bounds,
                        const vector<Rule>& rules, int dim) {
...
unsigned int SelectDimToSplit(const vector<vector<unsigned int>>& bounds,
  ...
  auto dimCost = [&rules, &bounds](int dim) -> double {
    ...
    for (Range s : segments) {
      int c = count_if(rules.begin(), rules.end(), [=](const Rule& r) -> bool {
        return r.range[dim][LowDim] <= s.first &&
               r.range[dim][HighDim] >= s.second;
      });
  ...
  for (int dim = 0; dim < rules[0].dim; dim++) {
    costs.emplace_back(dimCost(dim));
  }
...
unsigned int SelectSplitPoint(const vector<vector<unsigned int>>& bounds,
                              const vector<Rule>& rules, int dim) {
...
Node* SplitRules(const vector<vector<unsigned int>>& bounds,
                 const vector<Rule>& rules, unsigned int leafSize) {
  // PrintBounds(bounds);
  // CheckBounds(bounds);
...
void HyperSplitNode::SetBounds(
    const std::vector<std::vector<unsigned int>>& bounds) {
  CheckBounds(bounds);
  this->bounds = bounds;
}
  ...
  for (size_t i = 0; i < rules.size(); i++) {
    if (rules[i].priority > bestPriority) {
      ...
      for (int d = 0; d < rules[i].dim; d++) {
        if (!(rules[i].range[d][LowDim] <= p[d] &&
              rules[i].range[d][HighDim] >= p[d])) {
          matches = false;
          break;
        }
      }
