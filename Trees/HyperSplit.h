...
class Node {
 public:
  ...
  virtual void SetBounds(
      const std::vector<std::vector<unsigned int>>& bounds) = 0;
};
...
class NodeTable : public ClassifierTable {
 public:
  ...
  virtual bool CanInsertRule(const Rule& rule) const { return true; }

  virtual size_t NumRules() const { return numRules; }
  virtual int MaxPriority() const { return maxPriority; }
...
class HyperSplit : public PacketClassifier {
 public:
  HyperSplit(std::vector<std::vector<unsigned int>> bounds, int leafSize)
      : bounds(bounds), leafSize(leafSize) {}
  HyperSplit(int leafSize = 8) : leafSize(leafSize) {
    bounds.emplace_back(std::vector<unsigned int>{0, 0xFFFFFFFFu});
    bounds.emplace_back(std::vector<unsigned int>{0, 0xFFFFFFFFu});
    bounds.emplace_back(std::vector<unsigned int>{0, 0xFFFFu});
    bounds.emplace_back(std::vector<unsigned int>{0, 0xFFFFu});
    bounds.emplace_back(std::vector<unsigned int>{0, 0xFFu});
  }
  HyperSplit(const std::unordered_map<std::string, std::string>& args)
      : HyperSplit(GetIntOrElse(args, "HyperSplit.Leaf", 8)) {}
  virtual ~HyperSplit() { delete root; }
...
class HyperSplitNode : public Node {
 public:
   ...
   */
  HyperSplitNode(const std::vector<std::vector<unsigned int>>& bounds,
                 unsigned int split, int dim, Node* lc, Node* rc)
      : splitPoint(split),
        splitDim(dim),
        leftChild(lc),
        rightChild(rc),
...
class ListNode : public Node {
 public:
  // ListNode(const std::vector<std::vector<unsigned int>>& bounds) :
  // bounds(bounds) {}
  ListNode(const std::vector<std::vector<unsigned int>>& bounds,
...
Node* SplitRules(const std::vector<std::vector<unsigned int>>& bounds,
